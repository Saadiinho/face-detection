...
class FaceDetector:
    ...
    def _detect_eyes_fallback(self, img: np.ndarray) -> Tuple[List[Tuple[int, int, int, int]], float]:
        """
        ...
        eyes = self._eye_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(10, 10))

        # Détection yeux avec lunettes (pour renforcer)
        eyes_glasses = []
        if self._eye_glasses_cascade:
            eyes_glasses = self._eye_glasses_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10,
                                                                      minSize=(10, 10))
        ...
        return [(int(x), int(y), int(w), int(h)) for (x, y, w, h) in all_eyes], confidence
        ...
        try:
            ...
            if len(rects) == 0 and self.use_eye_fallback:
                ...
                if len(eye_rects) > 0:
                    ...
                    print(f"ℹ️ Repli sur détection d'yeux pour {image_path} ({len(eye_rects)} yeux trouvés)")
            ...
            return {
                "image": image_path,
                "has_face": len(faces_data) > 0,  # On garde has_face=True même si ce sont des yeux
...
class AdvancedFaceDetector:
    ...
    def analyze(self, image_path: str) -> Dict[str, Any]:
        ...
        if img is None:
            return {"image": image_path, "has_face": False, "face_count": 0, "confidence": 0.0,
                    "model_type": self.model_type, "faces": []}
        ...
        for face in faces:
            ...
            faces_data.append({"bbox": bbox, "confidence": score, "source": "retinaface"})
        ...
        return {
            ...
            "model_type": "eyes_fallback" if (
                        len(faces_data) > 0 and faces_data[0].get('source') == 'eyes_fallback') else self.model_type,
            "faces": faces_data
        }
